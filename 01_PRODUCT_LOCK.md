...
Literacy assumption: Telugu, Hindi, or English. Icon-first navigation. No screen in the core flow is English-only.
...
Telugu and Hindi locked text lives in `07_LOCALIZATION.md` and `l10n/app_te.arb` / `l10n/app_hi.arb`.
...
| Item | v1 |
|---|---|
| UI languages | `te`, `hi`, `en` |
| Fallback | English, with a debug log if a Telugu or Hindi key is missing |
