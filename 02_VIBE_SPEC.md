...
| ID | Pri | Requirement |
|---|---|---|
| FR-CS-01 | M | First launch shows language `te` / `hi` / `en`, each labelled in its own script. Persist in shared preferences. |
...
| FR-PDF-02 | M | On-device, < 5 s. Embed `NotoSans-Regular.ttf`, `NotoSansTelugu-Regular.ttf`, and `NotoSansDevanagari-Regular.ttf`. Locale `hi` must not tofu. |
...
Cut order if late: **S13–S20 skin/teeth** → S12 research → S10 polish → live sync (mock it) → third capture (allow stop at 2). Do not cut Hindi; it is in v1.
