...
Human unlock: Hindi (`hi`) is a v1 UI language. `l10n/app_hi.arb` (same keys as EN/TE), paper form `legal/consent_form_hi.md`, Noto Sans Devanagari in UI and PDF. S01 shows తెలుగు / हिन्दी / English. Locked `disclaimerFull` / `consentBody` / action strings written in Hindi; native-speaker + clinician initial still required before the volunteer study. Neonates, skin/teeth, and sync remain out.

### 1.3.1 — 2026-08-13

Machine-readable companions written and verified. `engineering/pubspec.yaml`, `l10n/*.arb` (camelCase), `vision/cielab_reference.py` (goldens Δ = 0.0000), FastAPI four-route stub, OpenAPI, local + server SQL, risk-shape SVGs, Dart Lab/classify reference. `06` algorithm pin corrected to `alg-1.1.0`. Secondary `HF_MODEL` left unset. `tools/verify_pack.py` is the pack go/no-go.
